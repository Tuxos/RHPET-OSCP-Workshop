...
Please have a look https://blog.openshift.com/deploying-applications-to-specific-nodes/[here^] if you want to have more information about this. In our ab, this is already done for you.
...
----
[root@workstation-835d ~]# oc login
...
. Click *View Full Log* to verify that a build is in progress.

. Review the log as the build progresses.

. Wait for the build to complete and use a browser to navigate to the overview page and click the `Routes - External Traffic`.
...
. On your browser, connect to the OpenShift web console from the http://seats.rhpet.de[Seat ID to GUID^] page.
.. Log in as `student` with the password `openshift`.
...
Please be aware that this Labguide is not completely tested with the environment, but it should work.
