...
Please have a look into the <<Appendix: Lab Environment Information>> for more information about the Lab evironment, like passwords, IP adresses and so on.
...
Login into OpenShift Dev environment as `admin` user with the password `openshift` (master-prod.example.com is the FQDN from the masternode of the OpenShift Prod enivronment. Look into the <<Appendix: Lab Environment Information>> for more information on IP addresses and FQDNs):
----
[root@workstation-6180 ~]# oc login https://master-prod.example.com:8443
...
master-prod.example.com   Ready     master    56d       v1.9.1+a0ce1bc657   beta.kubernetes.io/arch=amd64,beta.kubernetes.io/os=linux,kubernetes.io/hostname=master-dev.example.com,node-role.kubernetes.io/master=true,openshift-infra=apiserver,region=infra,zone=default
node01prod.example.com    Ready     compute   56d       v1.9.1+a0ce1bc657   beta.kubernetes.io/arch=amd64,beta.kubernetes.io/os=linux,kubernetes.io/hostname=node01dev.example.com,node-role.kubernetes.io/compute=true,region=primary,zone=main
node02prod.example.com    Ready     compute   56d       v1.9.1+a0ce1bc657   beta.kubernetes.io/arch=amd64,beta.kubernetes.io/os=linux,kubernetes.io/hostname=node02dev.example.com,node-role.kubernetes.io/compute=true,region=primary,zone=main
node03prod.example.com    Ready     compute   56d       v1.9.1+a0ce1bc657   beta.kubernetes.io/arch=amd64,beta.kubernetes.io/os=linux,kubernetes.io/hostname=node03dev.example.com,node-role.kubernetes.io/compute=true,region=primary,zone=main
----
...
+
----
[root@workstation-6180 ~]$ oc login https://master-prod.example.com:8443 -u student
----
...
----
[root@workstation-6180 ~]# ssh master-prod.example.com 'curl -s http://10.131.0.108:8080'
...
----
[root@workstation-6180 ~]$ cat << EOF > hello-many-pods.json
{
  "metadata":{
...
----
[root@workstation-6180 ~]# oc create -f hello-many-pods.json
...
----
[root@workstation-6180 ~]# oc delete -f hello-many-pods.json
...
----
[root@workstation-6180 ~]# oc new-project svcslab --display-name="Services Lab" --description="This is the project we use to learn about services"
...
----
[root@workstation-6180 ~]$ cat <<EOF > hello-service.json
{
  "kind": "Service",
  "apiVersion": "v1",
  "metadata": {
...
----
[root@workstation-6180 ~]$ oc create -f hello-service.json
...
----
[root@workstation-6180 ~]# oc get services
...
----
[root@workstation-6180 ~]# oc describe service hello-service
...
----
[root@workstation-6180 ~]# oc describe service hello-service|grep IP:|awk '{print $2}'
...
----
[root@workstation-6180 ~]# ssh master-prod.example.com 'curl -s http://172.30.69.150:8888'
...
----
[root@workstation-6180 ~]# oc new-project explore-example --display-name="Explore Example" --description="This is the project we use to learn about connecting to pods"
...
----
[root@workstation-6180 ~]# oc new-app --docker-image=openshift/hello-openshift:v1.5.1 -l "todelete=yes"
...
----
[root@workstation-6180 ~]# oc get svc
...
----
[root@workstation-6180 ~]# oc expose service hello-openshift --hostname=explore.apps-GUID.generic.opentlc.com
